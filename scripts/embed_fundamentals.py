...
and historical data filtering for fundamentals.
"""
...
class FundamentalsEmbedder:
    """Fundamentals Domain Embedding Pipeline with Setup Validation"""
    ...
    def load_data(self):
        """Load financial data from CSV files with setup validation"""
        # Load fundamentals
        ...
        labels_df_raw = pd.read_csv(labels_path)
        logger.info(f"Loaded {len(labels_df_raw)} raw label records")
        
        # Filter labels to only include confirmed setups
        self.labels_df = self.setup_validator.filter_labels_by_confirmed_setups(labels_df_raw)
        logger.info(f"Filtered to {len(self.labels_df)} confirmed setup labels")
    ...
    def create_records_with_labels(self, merged_df):
        """Create embedding records with performance labels and date validation"""
        records = []
        valid_records = 0
        filtered_historical = 0
        
        # Process each setup separately to ensure historical data filtering
        for setup_id in self.setup_validator.confirmed_setup_ids:
            setup_labels = self.labels_df[self.labels_df['setup_id'] == setup_id]
            
            if setup_labels.empty:
                continue
                
            # Get ticker for this setup
            ticker = setup_labels.iloc[0].get('stock_ticker', '')
            if not ticker:
                continue
            
            # Filter fundamentals data for this ticker to only include historical data
            ticker_data = merged_df[merged_df['ticker'] == ticker].copy()
            
            if ticker_data.empty:
                continue
            
            # Apply date filtering for fundamentals - only include data before setup date
            historical_data = self.setup_validator.filter_fundamentals_by_date(
                ticker_data, setup_id, date_column='period_end'
            )
            
            if historical_data.empty:
                logger.warning(f"No historical fundamentals data found for setup {setup_id}")
                continue
            
            # Create records for historical data only
            for idx, row in historical_data.iterrows():
                ...
                # Add performance labels from confirmed setup
                latest_label = setup_labels.iloc[-1]  # Use the setup-specific labels
                record.update({
            ...
            filtered_historical += len(historical_data)
        
        logger.info(f"Created {len(records)} valid records from {filtered_historical} historical fundamentals entries")
        logger.info(f"All records are for confirmed setups and use only historical data")
        return records
