...
def process_user_posts(
    ...
    Returns:
        Processed DataFrame with posts and labels
    """
    logger.info(f"Loading user posts from {posts_file}")
    
    # Load posts data
    posts_df = pd.read_csv(posts_file)
    logger.info(f"Loaded {len(posts_df)} user posts")
    
    # Load labels data
    labels_df = pd.read_csv(labels_file)
    logger.info(f"Loaded {len(labels_df)} performance labels")
    
    # Basic data validation
    required_posts_cols = ['post_id', 'setup_id', 'ticker', 'post_date', 'user_handle', 'post_content']
    missing_cols = [col for col in required_posts_cols if col not in posts_df.columns]
    if missing_cols:
